...
public static class Parser
{
    ...
    public static Widgettype ReadWidgetId(KaitaiStream input, out bool optionsFollow)
    {
        var widgetId = input.ReadUInt16();
        var widget = (Widgettype)(widgetId & ~32768);
        optionsFollow = (widgetId & 32768) == 0;
        //if (!Enum.IsDefined(typeof(RcpTypes.), option))
        //    throw new RCPDataErrorException("Parameter parsing: Unknown datatype!");
        return widget;
    }
    ...
    public static byte[] AddWidgetId(Widgettype widgetId, bool optionsFollow)
    {
        var wid = (ushort)widgetId;
        if (!optionsFollow)
            wid |= 32768;
        return BitConverter.GetBytes(wid);
    }
